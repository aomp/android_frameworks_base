...
public final class PowerManagerService extends IPowerManager.Stub
    ...
    public void notifyForceDisableAAL(int flag) {
        int eco_status = 0;
        if (flag > 0) {
            Settings.System.putInt(mContext.getContentResolver(),
                                    Settings.System.DISABLE_AAL, 1);
           if((mScreenBrightnessModeSetting & Settings.System.SCREEN_BRIGHTNESS_ECO_MODE_AUTOMATIC) > 0) {
               eco_status = 1;
               Settings.System.putInt(mContext.getContentResolver(),
                                       Settings.System.SCREEN_BRIGHTNESS_ECO_MODE, 0);
           }        
        } else {
            Settings.System.putInt(mContext.getContentResolver(),
                                    Settings.System.DISABLE_AAL, 0);
            if (1 == eco_status) {
                Settings.System.putInt(mContext.getContentResolver(),
                                        Settings.System.SCREEN_BRIGHTNESS_ECO_MODE, 2);
            }          
        }
    }
            ...
            synchronized (mLock) {
                ...
                if (mSbPlugState != sbPlugState) {
                    if (sbPlugState) {
                        ...
                        if (DEBUG_SPEW) {
                            Slog.d(TAG, "smart book mode: enable AAL when smart book plug in");
                        }
