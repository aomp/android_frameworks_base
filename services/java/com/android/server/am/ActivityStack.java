...
final class ActivityStack {
    ...
    final boolean moveTaskToBackLocked(int taskId, ActivityRecord reason) {
        ...
        /// M: ALPS01381200, fix can't resume home issue @{
        int numOfRunningTasks = 0;
        for (int i = 0; i < numTasks; i++) {
            final TaskRecord task = mTaskHistory.get(i);
            for (int n = 0; n < task.mActivities.size(); n++) {
                final ActivityRecord act = task.mActivities.get(n);
                if (!act.finishing) {
                    numOfRunningTasks++;
                    break;
                }
        ...
        if (numOfRunningTasks != numTasks) {
            Slog.w(TAG, "moveTaskToBackLocked: numOfRunningTasks=" + numOfRunningTasks + " didn't match numTasks=" + numTasks);
        }
        /// @}

        final TaskRecord task = mResumedActivity != null ? mResumedActivity.task : null;
        if (task == tr && task.mOnTopOfHome || numOfRunningTasks <= 1) {
