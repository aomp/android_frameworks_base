...
void DisplayList::outputViewProperties(const int level) {
    updateMatrix();
    if (mLeft != 0 || mTop != 0) {
        ALOGD("%*sTranslate (left, top) to (%d, %d) by property", level * 2, "", mLeft, mTop);
    }
    if (mStaticMatrix) {
    ...
    if (mMatrixFlags != 0) {
        if (mMatrixFlags == TRANSLATION) {
            ALOGD("%*sTranslate (%.2f, %.2f) by property", level * 2, "", mTranslationX, mTranslationY);
        } else {
    ...
    if (mAlpha < 1) {
        if (mCaching) {
            ...
            ALOGD("%*sSaveLayerAlpha "RECT_STRING", alpha %d, flag 0x%x by property", level * 2, "",
                    (float) 0, (float) 0, (float) mRight - mLeft, (float) mBottom - mTop,
                    (int)(mAlpha * 255), flags);
        }
    }
    if (clipToBoundsNeeded) {
        ALOGD("%*sClipRect "RECT_STRING" by property", level * 2, "", 0.0f, 0.0f,
...
void DisplayList::setViewProperties(OpenGLRenderer& renderer, T& handler,
    ...
    if (mLeft != 0 || mTop != 0) {
        ...
        DISPLAY_LIST_LOGD("%*sTranslate (left, top) to (%d, %d) by property ===> currTrans" MATRIX4_STRING,
            level * 2, "", mLeft, mTop, MATRIX4_ARGS(renderer.currentTransform()));
    }
    if (mStaticMatrix) {
        renderer.concatMatrix(mStaticMatrix);
        DISPLAY_LIST_LOGD("%*sConcatMatrix (static) %p: " MATRIX_STRING " by property ===> currTrans" MATRIX4_STRING,
                level * 2, "", mStaticMatrix, MATRIX_ARGS(mStaticMatrix), MATRIX4_ARGS(renderer.currentTransform()));
    } else if (mAnimationMatrix) {
        renderer.concatMatrix(mAnimationMatrix);
        DISPLAY_LIST_LOGD("%*sConcatMatrix (animation) %p: " MATRIX_STRING " by property ===> currTrans" MATRIX4_STRING,
                level * 2, "", mAnimationMatrix, MATRIX_ARGS(mAnimationMatrix), MATRIX4_ARGS(renderer.currentTransform()));
    }
    if (mMatrixFlags != 0) {
        if (mMatrixFlags == TRANSLATION) {
            renderer.translate(mTranslationX, mTranslationY);
            DISPLAY_LIST_LOGD("%*sTranslate (%.2f, %.2f) by property ===> currTrans" MATRIX4_STRING,
                level * 2, "", mTranslationX, mTranslationY, MATRIX4_ARGS(renderer.currentTransform()));
        } else {
            renderer.concatMatrix(mTransformMatrix);
            DISPLAY_LIST_LOGD("%*sConcatMatrix %p: " MATRIX_STRING " by property ===> currTrans" MATRIX4_STRING,
                    level * 2, "", mTransformMatrix, MATRIX_ARGS(mTransformMatrix), MATRIX4_ARGS(renderer.currentTransform()));
        }
    }
    ...
    if (mAlpha < 1) {
        if (mCaching) {
            ...
            clipToBoundsNeeded = false; // clipping done by layer
            DISPLAY_LIST_LOGD("%*sSetOverrideLayerAlpha %.2f by property", level * 2, "", mAlpha);
        } else if (!mHasOverlappingRendering) {
            renderer.scaleAlpha(mAlpha);
            DISPLAY_LIST_LOGD("%*sScaleAlpha %.2f by property", level * 2, "", mAlpha);
        } else {
            ...
            handler(mSaveLayerOp->reinit(0, 0, mRight - mLeft, mBottom - mTop,
                    ...
                    mClipToBounds);
            DISPLAY_LIST_LOGD("%*sSaveLayerAlpha "RECT_STRING", alpha %d, flag 0x%x <%p> by property",
                    level * 2, "", (float) 0, (float) 0, (float) mRight - mLeft, (float) mBottom - mTop,
                    (int)(mAlpha * 255), saveFlags, mSaveLayerOp);
        }
    }
    if (clipToBoundsNeeded) {
        ...
        DISPLAY_LIST_LOGD("%*sClipRect "RECT_STRING" <%p> by property ===> currClip(%.0f, %.0f, %.0f, %.0f)", level * 2, "", 0.0f, 0.0f,
                (float) mRight - mLeft, (float) mBottom - mTop, mClipRectOp, clipRect->left, clipRect->top, clipRect->right, clipRect->bottom);
    }
}
...
void DisplayList::iterate(OpenGLRenderer& renderer, T& handler, const int level) {
    ...
    if (g_HWUI_debug_display_list) {
        Rect* clipRect = renderer.getClipRect();
        DISPLAY_LIST_LOGD("%*sStart display list (%p, %s) ===> currClip(%.0f, %.0f, %.0f, %.0f)",
    ...
    if (mClipToBounds && renderer.quickRejectNoScissor(0, 0, mWidth, mHeight)) {
        ...
        DISPLAY_LIST_LOGD("%*sRestoreToCount %d (%p, %s) rejected by property", (level + 1) * 2, "", restoreTo, this, mName.string());
...
#if DEBUG_DISPLAY_LIST
            if (g_HWUI_debug_display_list) {
                op->output(level + 1, DisplayListOp::kOpLogFlag_MTK, &renderer);
            }
#endif
    }
