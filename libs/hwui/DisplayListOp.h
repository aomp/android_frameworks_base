...
public:
    ...
    virtual void output(int level, uint32_t logFlags = 0, OpenGLRenderer* renderer = NULL) const = 0;
...
public:
    ...
    virtual void output(int level, uint32_t logFlags, OpenGLRenderer* renderer) const {
...
public:
    ...
    virtual void output(int level, uint32_t logFlags, OpenGLRenderer* renderer) const {
        OP_LOG("SaveLayer%s of area " RECT_STRING ", alpha %d, mode %d, flags %d <%p>",
                (isSaveLayerAlpha() ? "Alpha" : ""), RECT_ARGS(mArea), mAlpha, mMode, mFlags, this);
    }
...
public:
    ...
    virtual void output(int level, uint32_t logFlags, OpenGLRenderer* renderer) const {
        if (logFlags & kOpLogFlag_MTK && renderer) {
            OP_LOG("Translate by (%.2f, %.2f) ===> currTrans" MATRIX4_STRING, mDx, mDy, MATRIX4_ARGS(renderer->currentTransform()));
        } else {
            OP_LOG("Translate by (%.2f, %.2f)", mDx, mDy);
        }
    }
...
public:
    ...
    virtual void output(int level, uint32_t logFlags, OpenGLRenderer* renderer) const {
        if (logFlags & kOpLogFlag_MTK && renderer) {
            OP_LOG("Rotate by %f degrees ===> currTrans" MATRIX4_STRING, mDegrees, MATRIX4_ARGS(renderer->currentTransform()));
        } else {
...
public:
    ...
    virtual void output(int level, uint32_t logFlags, OpenGLRenderer* renderer) const {
        if (logFlags & kOpLogFlag_MTK && renderer) {
            OP_LOG("Scale by (%.2f, %.2f) ===> currTrans" MATRIX4_STRING, mSx, mSy, MATRIX4_ARGS(renderer->currentTransform()));
        } else {
            OP_LOG("Scale by (%.2f, %.2f)", mSx, mSy);
        }
    }
...
public:
    ...
    virtual void output(int level, uint32_t logFlags, OpenGLRenderer* renderer) const {
        if (logFlags & kOpLogFlag_MTK && renderer) {
            OP_LOG("Skew by (%.2f, %.2f) ===> currTrans" MATRIX4_STRING, mSx, mSy, MATRIX4_ARGS(renderer->currentTransform()));
        } else {
            OP_LOG("Skew by (%.2f, %.2f)", mSx, mSy);
        }
    }
...
public:
    ...
    virtual void output(int level, uint32_t logFlags, OpenGLRenderer* renderer) const {
        if (mMatrix) {
            if (logFlags & kOpLogFlag_MTK && renderer) {
                OP_LOG("SetMatrix " MATRIX_STRING " ===> currTrans" MATRIX4_STRING, MATRIX_ARGS(mMatrix), MATRIX4_ARGS(renderer->currentTransform()));
            } else {
...
public:
    ...
    virtual void output(int level, uint32_t logFlags, OpenGLRenderer* renderer) const {
        if (logFlags & kOpLogFlag_MTK && renderer) {
            OP_LOG("ConcatMatrix " MATRIX_STRING " ===> currTrans" MATRIX4_STRING, MATRIX_ARGS(mMatrix), MATRIX4_ARGS(renderer->currentTransform()));
        } else {
...
public:
    ...
    virtual void output(int level, uint32_t logFlags, OpenGLRenderer* renderer) const {
        if (logFlags & kOpLogFlag_MTK && renderer) {
            ...
            OP_LOG("ClipRect " RECT_STRING " <%p> ===> currClip(%.0f, %.0f, %.0f, %.0f)", RECT_ARGS(mArea), this,
...
public:
    ...
    virtual void output(int level, uint32_t logFlags, OpenGLRenderer* renderer) const {
        ...
        if (logFlags & kOpLogFlag_MTK && renderer) {
            Rect* clipRect = renderer->getClipRect();
            OP_LOG("ClipPath bounds " RECT_STRING " <%p> ===> currClip(%.0f, %.0f, %.0f, %.0f)",
...
public:
    ...
    virtual void output(int level, uint32_t logFlags, OpenGLRenderer* renderer) const {
        ...
        if (logFlags & kOpLogFlag_MTK && renderer) {
            Rect* clipRect = renderer->getClipRect();
            OP_LOG("ClipRegion bounds %d %d %d %d <%p> ===> currClip(%.0f, %.0f, %.0f, %.0f)",
                bounds.left(), bounds.top(), bounds.right(), bounds.bottom(), this,
                clipRect->left, clipRect->top, clipRect->right, clipRect->bottom);
        } else {
...
#define SET_TEXTURE(ptr, posRect, offsetRect, texCoordsRect, xDim, yDim) \
    ...
    virtual void output(int level, uint32_t logFlags, OpenGLRenderer* renderer) const {
        OP_LOG("Draw bitmap %p at "RECT_STRING" <%p>", mBitmap, RECT_ARGS(mLocalBounds), this);
    }
...
public:
    ...
    virtual void output(int level, uint32_t logFlags, OpenGLRenderer* renderer) const {
        OP_LOG("Draw bitmap %p matrix "MATRIX_STRING" at "RECT_STRING" <%p>",
            mBitmap, MATRIX_ARGS(mMatrix), RECT_ARGS(mLocalBounds), this);
    }
...
public:
    ...
    virtual void output(int level, uint32_t logFlags, OpenGLRenderer* renderer) const {
        OP_LOG("Draw bitmap %p data at "RECT_STRING" <%p>", mBitmap, RECT_ARGS(mLocalBounds), this);
    }
...
public:
    ...
    virtual void output(int level, uint32_t logFlags, OpenGLRenderer* renderer) const{
        if (CC_LIKELY(mPaint)) {
            OP_LOG("Draw Rect "RECT_STRING", paint %p, id %d, style %d, AA %d, color 0x%x <%p>", RECT_ARGS(mLocalBounds), mPaint,
                            mPaint->getGenerationID(), mPaint->getStyle(), mPaint->isAntiAlias(), mPaint->getColor(), this);
        } else {
    ...
    virtual status_t multiDraw(OpenGLRenderer& renderer, Rect& dirty,
        ...
        if (CC_LIKELY(mPaint)) {
            ALOGD("drawRect multiDraw %s paint %p, id %d, style %d, AA %d, color 0x%x", (ops[0].op)->viewName.string(),
                mPaint, mPaint->getGenerationID(), mPaint->getStyle(), mPaint->isAntiAlias(), mPaint->getColor());
        }
#endif
...
public:
    DrawSomeTextOp(const char* text, int bytesCount, int count, SkPaint* paint)
            : DrawOp(paint), mText(text), mBytesCount(bytesCount), mCount(count), mLogText(NULL) {

        if (g_HWUI_debug_display_list) {
            int total = mCount;
            SkUnichar *tmpText = (SkUnichar*)malloc(total*sizeof(SkUnichar));
            memset(tmpText, 0, total*sizeof(SkUnichar));
            mPaint->glyphsToUnichars((uint16_t*)mText, mCount, tmpText);
            total = utf32_to_utf8_length((char32_t*)tmpText, mCount) + 1;
            mLogText = (char*)malloc(total*sizeof(char));
            memset(mLogText, 0, total*sizeof(char));
            utf32_to_utf8((char32_t*)tmpText, mCount, mLogText);
            free(tmpText);
        }
    };

    ~DrawSomeTextOp() {
        if (mLogText != NULL) free(mLogText);
    }

    virtual void output(int level, uint32_t logFlags, OpenGLRenderer* renderer) const {
        OP_LOG("Draw some text \"%s\", %d bytes <%p>", mLogText, mBytesCount, this);
    }
...
public:
    DrawTextOp(const char* text, int bytesCount, int count, float x, float y,
            ...
            mX(x), mY(y), mPositions(positions), mTotalAdvance(totalAdvance), mLogText(NULL) {
    ...
    virtual void output(int level, uint32_t logFlags, OpenGLRenderer* renderer) const {
        if (CC_LIKELY(mPaint)) {
            OP_LOG("Draw Text \"%s\", count %d, bytes %d, alpha %d, color 0x%x, paint %p at " RECT_STRING " <%p>",
                mLogText, mCount, mBytesCount, mPaint->getAlpha(), mPaint->getColor(), mPaint, RECT_ARGS(mLocalBounds), this);
        } else {
...
public:
    ...
    virtual void output(int level, uint32_t logFlags, OpenGLRenderer* renderer) const {
        /// M: comment out this log because it's duplicate to "Start display list ...."
...
public:
    ...
    virtual void output(int level, uint32_t logFlags, OpenGLRenderer* renderer) const {
        OP_LOG("Draw Layer %p at (%.2f, %.2f), textureSize (%d, %d), layerSize (%.2f, %.2f), isIdentity %d, isBlend %d, isTextureLayer %d<%p>",
            mLayer, mX, mY, mLayer->getWidth(), mLayer->getHeight(), mLayer->layer.getWidth(), mLayer->layer.getHeight(),
            mLayer->getTransform().isIdentity(), mLayer->isBlend(), mLayer->isTextureLayer(), this);
    }
